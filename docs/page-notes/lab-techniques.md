...
Six dead/stale vendor links in the AOM/EOM/power-supply vendor cards, fixed with independently-verified replacements: Qubig EOM page (404 → `qubig.com/products/electro-optic-modulators_eom`), Jenoptik EOM page (404 → `jenoptik.com/products/optoelectronic-systems/light-modulation`), AMETEK Programmable Power (domain dead, company rebranded → `programmablepower.com`), MOGLabs CEL laser (MOGLabs acquired by Santec; followed to the correct product-matched page, not the naive redirect target, which pointed at a different MOGLabs product line → `inst.santec.com/products/quantum-instruments/quantum-lasers/cel`), and Laser Components laser-diodes page (404 → `lasercomponents.com/us/products/emitters/`, 2 occurrences). Also fixed a label/href mismatch in the "Bias coil drivers" card: the link text read "itech" (a different vendor) while the href pointed at B&K Precision's now-dead `categories/dc-power-supplies` path — updated the href to their current `products/power-supplies` page and relabeled the text to "B&K Precision" to match reality. Gooch & Housego's AOM page also 404'd, but no trustworthy replacement could be verified (site blocks automated fetchers on most paths) — de-linked to plain text per the site's established dead-link-with-no-replacement precedent, flagged for a manual re-link. See docs/link-audit-2026-09-16.md.


## Technical audit update (2026-09-16)
`og:title`, `twitter:title`, and the JSON-LD `headline` still said the page's old name, "Lab Techniques" -- stale from whenever this page was renamed to "AMO Lab Operations Handbook" in `<title>`, the on-page content, and `main.js`'s `NAV` entry/home-page card. Synced all three meta fields to the current name, plus the matching reference in `llms.txt`/`llms-full.txt` and `CLAUDE.md`'s per-page section header. See docs/technical-audit-2026-09-16.md.
