...
- **Second-source cross-check, §05 callout:** user supplied Fox's *Quantum Optics: An Introduction* Fig. 9.10 (Bloch-sphere T1/T2' damping figure, Ch. 9 per the user's recollection — not independently verified against a local copy the way Steck was, since no Fox source file exists in this environment; flagged as such to the user). Added a callout mapping Fox's $T_2'$ notation onto this page's $T_\phi$/$\gamma_c$ (same relation, $1/T_2=1/(2T_1)+1/T_2'$), plus Fox's caption point that the relaxed Bloch vector is only meaningful ensemble-averaged, not per-atom — generalizes the existing §04 callout to §03 as well and foreshadows a possible future "quantum trajectories vs. master equation" companion page.
...
- Added as tools-grid card **`QC 01`** on `home.html` — this slot was unclaimed (existing cards jumped straight to `QC 02`). Existing `QC 02/03/04` cards were left untouched rather than renumbered. (Update, later pass: `rb-explorer` and `dd-playground` — the two quantum-category pages that still had no tools-grid card — were given `QC 05`/`QC 06` cards; see their own page-notes. `learn-quantum` remains card-less by design, since it has its own dedicated teaser section instead.)
