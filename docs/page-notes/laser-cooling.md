...
## Interactive-correctness audit update (2026-09-22)
This page was never actually covered by the original interactive-correctness audit (2026-09-08) despite having three real physics calculators (Lamb-Dicke, Sideband Cooling Limit, Gray Molasses) — a coverage gap discovered during this audit's rotation re-check. The Lamb-Dicke Parameter Calculator (`calcLD()`) had a real bug: it plugged the user-entered "trap frequency ν (kHz)" (ordinary/measured frequency, as used everywhere else on this page and site-wide) directly into `x0 = √(ħ/2mν)`, which is only valid for angular frequency ω=2πν — missing that conversion overstated η by √(2π)≈2.51× and the recoil-energy ratio by 2π (self-inconsistent with the page's own stated typical η≈0.05–0.15 range, and with the *correctly*-implemented identical formula already on `tweezer-designer.html`/`cooling-simulator.html`). Fixed `calcLD()` to convert ν→ω before use, and updated the three adjacent KaTeX formula boxes (Trap Hamiltonian, zero-point motion, Lamb-Dicke parameter) plus one prose sentence to match. The other two calculators on this page (Sideband Cooling, Gray Molasses) were checked and are correct. See docs/interactive-audit-2026-09-22.md.
