...
```
`.tool-card-num` reads `"{CATEGORY} {NN}"` — must match the tool's position in its `NAV` category array. The `home.html` footer has 3 `.footer-links` columns, not a 1:1 mirror of the 5 `NAV` categories: **Build** (all 10 `build` tools), **Measure & QC** (the 5 `measure` tools plus `absorption-imaging` and most of `quantum`: `fidelity-budget`, `decoherence-lab`, `rb-explorer`, `dd-playground`, `remote-entanglement`), and **Learn & Career** (the 2 `cooling` tools, the remaining 2 `quantum` tools — `rydberg-calculator`, `learn-quantum` — and all 7 `career` tools). When adding a tool, add its `.footer-link` to whichever of these 3 columns its siblings are already in (a navigation/IA audit — 2026-09, re-checked 2026-09-23 — confirmed every existing tool has exactly one footer link; check `docs/navigation-audit-2026-09-23.md` if this grouping seems to drift again).
